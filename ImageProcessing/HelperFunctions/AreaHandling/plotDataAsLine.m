...
function plotAreaLine(area_data_array,file_name_short) % similar function as 'plotArea.m', but plots the data as a line instead of as a scatter plot
    ...
    cell_array_size = size(area_data_array); % get the size of the cell array inpu
    len = cell_array_size(2); % get the number of dataset inputs
    color = {'b','r','g','k','c','m','y'}; % cell array of line colors to loop through;

    for i=1:len % iterate through each dataset
        figure(fhandle) % plot everything on the same set of axes
        time = area_data_array{i}(2,:);
        area = area_data_array{i}(3,:);
        plot(time,area,'LineWidth',2,'Color',color{i}); % plot each data series
    end
