...
function [clean_label_img, wet_frac] = countArea(orig_mask,outer_region,max_film_area,img_size)
    ...
    dewet_frac = total_dewet_area / max_film_area; % fraction of dewetted area, excluding the shadow's area
...
function mask = edgeAlg(mask,outer_region,linear_indices,component_mask)
    ...
    overlap = 10; % number of pixels overlap
    if (nnz(union) <= overlap) % if the component has little overlap with the outer region
...
%         cond2 = stats(c).EulerNumber < -20 && stats(c).Area < 5000; % medium objects with a large number of holes
    ...
    % TODO: use 'bwpropfilt' instead
%     low = -50;
%     high = 10;
%     mask = bwpropfilt(mask,'EulerNumber', [low high]);
