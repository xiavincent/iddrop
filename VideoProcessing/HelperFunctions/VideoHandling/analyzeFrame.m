...
function [wet_frac,overlay_img] = analyzeFrame(input_vid,analys,fnum,params)
    scaled_mask = analys.scaled_mask; % redefine parameters
    ...
    [overlay_img,wet_frac] = getFilmOverlay(crop,scaled_mask,area_mask,max_area,fnum,area_fnum,seed_rc); % get wet film mask from area frame
...
function [overlay,wet_frac] = getFilmOverlay(RGB_img,scaled_mask,area_mask,max_area,fnum,area_fnum,seed_rc)  
    film_mask = findFilm(RGB_img,area_mask,seed_rc); % return a mask of the wet film
