...
% check the area to make sure it's reasonable
% set it to '1' if not
function wet_fraction = checkArea(wet_fraction,cur_framenum,area_framenum)
    if ((wet_fraction < .5) && (cur_framenum < area_framenum)) % if the video dewetted abnormally quickly
        wet_fraction = 1; % reset the area fraction
    end
end
