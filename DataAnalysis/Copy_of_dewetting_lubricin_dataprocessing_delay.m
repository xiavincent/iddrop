...
fname=fname(1:end-4); %removes .txt or other file extension from filename
...
%     onset_time_indices = find(areadata < upperbound );

    t = 100; % define delay time point after which you want to start the analysis
    A(2,:) = areadata;
    onset_time_indices = find(A(2,:) < upperbound & A(1,:) > t);
    startpoint = onset_time_indices(1);

    secondhalf = find(A(2,:) < lowerbound & A(1,:) > t);
