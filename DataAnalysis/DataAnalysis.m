...
fileID = fopen(fname, 'r'); % open file for reading
Wet_area = readmatrix(fname);

% formatSpec = '%f'; % search for floating point numbers
% [Wet_area,count] = fscanf(fileID, formatSpec); % search for floating point numbers
...
% (comment out this section if all you want is the smoothed graph)
    ...
    onset_time_indices = find(Wet_area(:,3) < upperbound & Wet_area(:,2) > t);
    startpoint = onset_time_indices(1);

    secondhalf = find(Wet_area(:,3) < lowerbound & Wet_area(:,2) > t);
